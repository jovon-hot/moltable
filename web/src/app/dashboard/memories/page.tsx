...
export default function MemoriesPage() {
  ...
  const handleSearch = () => {
    ...
    setOffset(0)
    fetchMemories(search || undefined, category)
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-10">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-heading tracking-[-0.3px] text-ln-text">记忆管理</h1>
          <p className="text-sm text-ln-tertiary font-body mt-1">AI 自动积累的记忆片段，也可手动管理</p>
        </div>
        <button
          onClick={() => { if (isDemo) { showDemoToast(); return }; setShowAddForm(!showAddForm) }}
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        <select
          ...
          onChange={e => {
            ...
            setOffset(0)
            fetchMemories(search || undefined, e.target.value)
          }}
      ...
      {isDemo && (
        <div className="mb-6 px-4 py-3 rounded-card bg-ln-accent-muted text-ln-accent-hover text-sm font-body shadow-border-accent animate-in">
          🔍 演示数据 — <a href="/login" className="underline font-ui">注册后</a> 可管理真实记忆
        </div>
      )}
      ...
      {loading && memories.length === 0 ? (
        ...
        <div className="text-center py-20 rounded-card shadow-[0_0_0_1px_dashed_rgba(255,255,255,0.08)]">
          ...
          <p className="text-base mb-2 text-ln-text font-ui">暂无记忆</p>
          <p className="text-sm mb-6 text-ln-tertiary font-body">连接 AI Agent 后会自动积累，也可以手动添加。</p>
          <button
            onClick={() => { if (!isDemo) setShowAddForm(true) }}
        ...
        <div className="space-y-2">
          {memories.map((m: any) => {
            ...
            return (
              <div
                key={m.id}
                className={`group p-4 rounded-card bg-ln-surface transition-all duration-200 ${
                  isEditing ? 'shadow-accent-glow' : 'shadow-card hover:shadow-card-hover'
                }`}
              >
                <div className="flex items-start justify-between gap-4">
                  {/* Category color stripe */}
                  <div className={`flex-shrink-0 w-1 h-full min-h-[3rem] rounded-full mt-0.5 ${
                    m.category === 'preference' ? 'bg-[#eab308]' :
                    m.category === 'decision' ? 'bg-[#4ade80]' :
                    m.category === 'fact' ? 'bg-[#60a5fa]' :
                    m.category === 'project' ? 'bg-[#818cf8]' : 'bg-ln-tertiary'
                  }`} />
                    ...
                    {isEditing ? (
                      <div className="space-y-2">
                        <textarea
                          ...
                          rows={3}
                          className="w-full px-3 py-2 rounded-btn bg-ln-bg text-ln-text text-sm font-body shadow-border-accent outline-none resize-y animate-in"
                          autoFocus
                        />
                        <div className="flex gap-2">
                          <button
                      ...
                      <>
                        ...
                        <div className="flex flex-wrap items-center gap-2 mt-2">
                          <span className={`text-xs px-2 py-0.5 rounded-full ${
                            catColor ? `${catColor.bg} ${catColor.text} ${catColor.border}` : 'bg-ln-btn-bg text-ln-tertiary shadow-border-subtle'
                          }`}>
                            {m.category}
                          </span>
                          {m.source && (
                            <span className="text-xs px-2 py-0.5 rounded-full bg-ln-btn-bg text-ln-tertiary shadow-border-subtle">
                              {m.source}
                            </span>
                          )}
                          {m.similarity !== undefined && (
                            <span className="text-xs px-2 py-0.5 rounded-full bg-ln-accent-muted text-ln-accent-hover shadow-border-accent">
                              {Math.round(m.similarity * 100)}% 匹配
                            </span>
                          )}
                          {m.relevance !== undefined && (
                            <span className="text-xs px-2 py-0.5 rounded-full bg-ln-accent-muted text-ln-accent-hover shadow-border-accent">
                              {Math.round(m.relevance * 100)}% 匹配
                            </span>
                          )}
                  ...
                  {!isEditing && (
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                      <button
                        onClick={() => startEdit(m)}
                        className="p-1.5 rounded-btn text-ln-tertiary hover:text-ln-accent-hover hover:bg-ln-accent-muted transition-all duration-150"
                        aria-label="编辑"
                      >
                        <Edit2 size={13} />
                      </button>
                      <button
                        onClick={() => { if (window.confirm('确定删除这条记忆？')) handleDelete(m.id) }}
                        className="p-1.5 rounded-btn text-ln-tertiary hover:text-ln-error hover:bg-ln-error/10 transition-all duration-150"
                        aria-label="删除"
                      >
                        <Trash2 size={13} />
                      </button>
